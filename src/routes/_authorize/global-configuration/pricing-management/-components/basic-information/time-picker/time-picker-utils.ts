...
export function set12Hours(date: Date, value: string, period: Period) {
  ...
  date.setUTCHours(convertedHours)
  return date
}
...
export function getDateByType(date: Date, type: TimePickerType) {
  switch (type) {
    case "minutes":
      return getValidMinuteOrSecond(String(date.getUTCMinutes()))
    case "seconds":
      return getValidMinuteOrSecond(String(date.getUTCSeconds()))
    case "hours":
      return getValidHour(String(date.getUTCHours()))
    case "12hours": {
...
export const fromCalendarDate = (
  ...
  const hour = prev ? prev.getUTCHours() : 0
  const minute = prev ? prev.getUTCMinutes() : 0
  const second = prev ? prev.getUTCSeconds() : 0
